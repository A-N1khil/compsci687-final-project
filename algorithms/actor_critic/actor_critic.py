...
#         # Edit 1:
#         # starting with separate networks increased parameters and slowed training
#         # moving to shared encoder so both heads learn from the same state features
...
#         # Edit 2:
#         # increasing hidden_dim to 128 to give more capacity on continuous MountainCar
...
#     #     # computing discounted returns G_t using backward accumulation
#     #     returns = []
#     #     G = 0.0  # running sum
#     #     for r in reversed(rewards):  # iterating backwards to fold in future rewards first
#     #         G = r + self.gamma * G
#     #         returns.insert(0, G)  # prepending to keep time order
#     #     return torch.tensor(returns, dtype=torch.float32).to(self.device)
    
#     def compute_returns(self, rewards, last_value, done):
#         returns = []
#         G = 0 if done else last_value  # bootstrap if not terminal
...
#         #     # interacting with environment using raw MountainCar reward
#         #     next_state, reward, done, _ = self.mdp.step(action)

#         #     # storing episode data for learning
#         #     log_probs.append(log_prob)
#         #     values.append(value)
#         #     rewards.append(reward)        # keeping pure env reward (no shaping)
...
#         # get final value
#         last_value = self.net.forward_pass(torch.from_numpy(state).float().to(self.device))[1].item()
...
#             log_probs, values, rewards, entropies,last_value,done = self.run_episode(max_steps)
#             returns = self.compute_returns(rewards, last_value, done)

#             loss = self.compute_loss(log_probs, values, returns, entropies)

#             episode_reward = sum(rewards)  # using raw env return as performance metric
...
#             if episode % 20 == 0:
#                 with torch.no_grad():
#                     # sample a state from beginning of episode
#                     test_state = torch.from_numpy(normalize_state(self.mdp.reset())).float().unsqueeze(0).to(self.device)
#                     test_logits, test_value = self.net.forward_pass(test_state)
